...
class TrackingEvaluator:
    ...
    def get_overall_performance(self):
        ...
        effective_tracking_percent = effective_tracking / total_frames
        avg_similarity = np.mean(valid_similarities) if valid_similarities else 0

        # Composite score that prioritizes longer successful tracking
        composite_score = (effective_tracking_percent * 0.7) + (avg_similarity * 0.3)

        avg_metrics = {
            'avg_appearance_similarity': np.mean(valid_similarities) if valid_similarities else 0,
            'avg_bbox_jitter': np.mean(valid_jitters) if valid_jitters else float('inf'),
...
def generate_comparison_charts(metrics_results, video_path):
    ...
    for i, (metric_name, config) in enumerate(metrics_to_plot.items(), 1):
        ...
        # For appearance similarity, higher is better, so invert the bars
        if metric_name == 'avg_appearance_similarity':
            colors = ['green' if v > 0.7 else 'orange' if v > 0.5 else 'red' for v in values]
        elif metric_name == 'failure_rate':
            colors = ['green' if v < 0.1 else 'orange' if v < 0.3 else 'red' for v in values]
        else:
            colors = 'skyblue'
        ...
        for bar in bars:
            ...
            label_text = f"{height:.1f}%" if config['percentage'] else f"{height:.1f}"
    ...
    with open(f"{output_base}_metrics.csv", 'w') as f:
        # Write header
    ...
    print(f"Metrics data saved to {output_base}_metrics.csv")
